...
def main():
    ...
    parser.add_argument("--bootstrap_opt", type=str, nargs="*",
                        dest="bootstrapOpt", required=True,
                        help="Do use bootstrap samples for optimizers that "
                             "include this string")
    ...
    if args.bestof == "train":
        ...
        args.est_train = True
        args.test = False
    trn_performance, _, trn_name_list = read_traj_files(args)
    ...
    for name in name_list:
        ...
        if sum([r in name for r in args.bootstrapOpt]) > 0:
            trn_p = trn_performance[old_indx]
            # draw bootstrap samples
            print("Bootstrap %s" % name)
