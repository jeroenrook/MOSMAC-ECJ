...
#else
    printf("GpG.max_plan_length is %d!", GpG.max_plan_length);
  ...
  **/
  ...
  if ( GpG.remove_actions_in_next_step ) {
	...
	if (ind_remove_act_chain_next_step >= MAX_PLAN_LENGTH) {
	  ...
	  MSG_ERROR( WAR_MAX_PLAN_LENGTH );
#else
    	printf("ind_remove_act_chain_next_step is %d!", ind_remove_act_chain_next_step);
	  printf( WAR_MAX_PLAN_LENGTH );
#endif    
	  exit (1);
	}
  ...
  if (i == ind_remove_act_chain)
	...
	MSG_ERROR( WAR_MAX_PLAN_LENGTH );
#else
    	printf("ind_remove_act_chain is %d!", ind_remove_act_chain);
 	printf( WAR_MAX_PLAN_LENGTH );
#endif    
	exit (1);
      }
