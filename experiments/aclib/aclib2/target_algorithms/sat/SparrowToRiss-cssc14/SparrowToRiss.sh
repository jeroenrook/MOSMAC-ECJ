...
binaryPath=target_algorithms/sat/SparrowToRiss-cssc14/
...
then
	...
	then 
		...
		then
			...
			echo "c sparrow command line: $binaryPath./$satsolver $flipParameter $secondsParameter $sparrowParams $tmpCNF $seed" 1>&2
			solveStart=`date +%s`
			...
			# and output to stdout of the sat solver is redirected to stderr
			#
			echo "c sparrow command line: $binaryPath./$satsolver $flipParameter $secondsParameter $sparrowParams $file $seed" 1>&2
			solveStart=`date +%s`
