...
const char *arg_dlb;
#define DEFINE_OPTIONS_PARAMETER(A, B, C, D, TYPE) DEFINE_PARAMETER(A,B,C,D,TYPE)
#define DEFINE_PARAMETER(A, B, C, D, TYPE)                          \
    /* Set to 1 if option A has been specified.  */                 \
    unsigned int opt_##A : 1;                                       \
    /* Argument to option A.  */                                    \
    const char *arg_##A;                                            \

#include "acotsp-parameters.def"
};
...
static const char *const STR_HELP[] = {
    ...
    STR_HELP_HELP ,
#define DEFINE_OPTIONS_PARAMETER(A, B, C, D, TYPE) DEFINE_PARAMETER(A,B,C,D,TYPE)
#define DEFINE_PARAMETER(PARAM, SHORT, LONG, HELP, TYPE) \
    "  " SHORT ", --" LONG "\t" HELP "\n",
#include "acotsp-parameters.def"
    NULL
};
...
#define DEFINE_OPTIONS_PARAMETER(A, B, C, D, TYPE) DEFINE_PARAMETER(A,B,C,D,TYPE)
#define DEFINE_PARAMETER(A, B, C, D, TYPE)                                     \
    static const char *const optstr__##A = C; 
#include "acotsp-parameters.def"
...
#include "acotsp-parameters.def"
  ...
  while (++i < argc)
  {
    ...
    else if (*option == '-')
    {
      ...
      case 'v':
          HANDLE_LONG_OPTION(var_n_ants)
          HANDLE_LONG_OPTION(var_beta)
          HANDLE_LONG_OPTION(var_rho)
          HANDLE_LONG_OPTION(var_q0)
          goto error_unknown_long_opt;
...
#define ASSIGN_ENUM_PARAMETER(PARAM, TYPE)                                     \
    do {                                                                       \
        ...
        {                                                                      \
            ...
            option_##PARAM = opt;                                              \
            fprintf(stdout, " %s  --%s ", "", optstr__##PARAM);                \
            if (options.arg_##PARAM) {                                         \
                printf ("with argument \"%s\"\n", strings_##TYPE[opt]);        \
            }                                                                  \
        } else {                                                               \
            fprintf(stdout,"\tNote: \"--%s\" is set to default \"%s\"\n",      \
                    optstr__##PARAM, strings_##TYPE[option_##PARAM]);          \
        }                                                                      \
    } while(0)                                                                 \
        
    
#define ASSIGN_DOUBLE_PARAMETER(PARAM)                                         \
    do {                                                                       \
        if ( options.opt_##PARAM ) {                                           \
...
#define ASSIGN_INTEGER_PARAMETER(PARAM)                                        \
    do {                                                                       \
        if ( options.opt_##PARAM ) {                                           \
...
#define DEFINE_DOUBLE_PARAMETER(PARAM) ASSIGN_DOUBLE_PARAMETER(PARAM);
#define DEFINE_INTEGER_PARAMETER(PARAM) ASSIGN_INTEGER_PARAMETER(PARAM);
...
#include "acotsp-parameters.def"
    ...
    if ( options.opt_beta ) {
	opt_beta = atof(options.arg_beta);
	fputs ("  -b  --beta ", stdout);
	if (options.arg_beta != NULL)
	    printf ("with argument \"%f\"\n", opt_beta);
	check_out_of_range( opt_beta, 0., 100., "beta");
    } else {
    ...
    if ( options.opt_rho ) {
	opt_rho = atof(options.arg_rho);
	fputs ("  -e  --rho ", stdout);
	if (options.arg_rho != NULL)
	    printf ("with argument \"%f\"\n", opt_rho);
	check_out_of_range( opt_rho, 0.000001, 1., "rho");
    } else {
	fprintf(stdout,"\tNote: rho is set to default %g\n", opt_rho);
    }

    check_out_of_range( end_rho, 0.000001, 1., "end_rho");

    if ( options.opt_q0 ) {
	opt_q0 = atof(options.arg_q0);
	fputs ("  -q  --q0 ", stdout);
	if (options.arg_q0 != NULL)
	    printf ("with argument \"%f\"\n", opt_q0);
        check_out_of_range( opt_q0, 0., 1., "q0");
    } else {
