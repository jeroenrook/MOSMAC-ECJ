...
int main(int argc, char *argv[])
{
    ...
    for (Run = 1; Run <= Runs; Run++) {
        ...
        //YP: I edited the following line ot remove '&& MaxPopulationSize >= 1'
        //so that the algorithm will now terminate correclty when it has found
        //the optimal solution.
        if (StopAtOptimum && Cost == OldOptimum) {
            Runs = Run;
            break;
        }
        if (PopulationSize >= 2 &&
