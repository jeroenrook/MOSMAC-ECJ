...
public:
  ...
  int fBestValueOverall;                 /* The tour lenght of the best tour in the population (across several soft restarts) */
  double timeToFindBestSol;              /* Record the time to find best solution so far (across several soft restarts) */
  double timeLastRunTook;                /* Record the time the last run took (easier to print it later) */
...
// hh added:
  int fTargetTourLength;							/* tour length at which a run of EAX will be terminated */
  double fCutoffTime; 								/* cutoff time for each run (= trial) in seconds */
	int rndSeed;												/* random number seed */
	int noFurtherRestarts;							/* flag indicating when no further restarts should be performed */
// hh end
