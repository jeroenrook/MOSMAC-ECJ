...
targetErrorMode              s    ""     "--target-error-mode"          "quit"             "What to do when a call to target-runner or target-evaluator fails. Valid values are: \'quit\' terminate irace immediately, \'stop\' irace is put to sleep with signal SIGSTOP until the user sends a SIGCONT signal (not available in Windows), \'retry\' retries without sleeping after (see --target-runner-retries)."
...
aclib                        b    ""     "--aclib"                     0    "Enable/disable AClib mode. This affects the input that irace accepts and the output printed."
runlog                       s    ""     "--runlog"                    "./irace-runlog.txt"    "Output file to print all evaluations done in AClib format."
dumpAllConfigurations        s    ""     "--dump-all-configurations"                    ""    "Write all configurations generated to this file."
')
