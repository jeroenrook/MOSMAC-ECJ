...
{
  return (floor(2 + log2(nbParameters)))
}
                                    ...
                                    nElites = 0, nOldInstances = 0, newInstances = 0)
{
  ...
  # We allow enough budget to do a single test on the first iteration, two
  # tests on the second, three tests on the third.
  n <- max (firstTest + eachTest * min(5, indexIteration - 1),
...
print.trajectory <- function(old.time, cpu.time,
                             evaluations, cost, configuration, parameters)
{
  new.time <- proc.time()
  total.cpu.time <- (new.time[["user.self"]] + new.time[["sys.self"]]) -
    (old.time[["user.self"]] + old.time[["sys.self"]])
  wc.time <- new.time[["elapsed"]] - old.time[["elapsed"]]

  configuration <- removeConfigurationsMetaData(configuration)
  # Re-sort the columns
  configuration <- configuration[, unlist(parameters$names), drop = FALSE]
  configuration <- buildCommandLine(configuration[1, , drop=FALSE], parameters$switches)

  cat(sep = "",
      '{"cpu_time":', cpu.time, ', "total_cpu_time":', total.cpu.time,
      ', "wallclock_time":', wc.time, ', "evaluations":', evaluations,
      ', "cost":', cost, ', "incumbent":"', configuration, '"}\n')
}
...
irace <- function(scenario, parameters)
{
  ...
  if (!is.null(scenario$recoveryFile)) {
    ...
    if (scenario$maxTime == 0) {
      ...
      while (TRUE) {
        ...
        cat ("# nconfigurations:", nconfigurations, "\n")
        cat ("# next.conf:", next.configuration, "\n")
        # Execute tests
    ...
    # Remove runlog file to avoid appending to an existing file.
    if (scenario$aclib && !is.null.or.empty(scenario$runlog)
        && file.exists(scenario$runlog))
      file.remove(scenario$runlog)
  }
  iraceResults$timeStart <- proc.time()
  ...
  while (TRUE) {
    ...
    if (nbConfigurations <= minSurvival) {
      ...
      break # return (eliteConfigurations)
    }
    ...
    if (scenario$elitist) {
      ...
      if ((nbConfigurations - nrow(eliteConfigurations)) * max(scenario$mu, scenario$firstTest)
          ...
          > currentBudget) {
        catInfo("Stopped because there is no enough budget left to race all configurations up to the first test (or mu)")
        break # return (eliteConfigurations)
      }
    ...
    if (scenario$aclib) {
      print.trajectory (iraceResults$timeStart, timeUsed, experimentsUsedSoFar,
                        configuration = eliteConfigurations[1, , drop = FALSE],
                        cost = mean (iraceResults$experiments[, as.character(eliteConfigurations$.ID.[1])],
                          na.rm = TRUE),
  ...
  if (!is.null.or.empty(scenario$dumpAllConfigurations))
    write.table(removeConfigurationsMetaData(allConfigurations),
                file = scenario$dumpAllConfigurations)
