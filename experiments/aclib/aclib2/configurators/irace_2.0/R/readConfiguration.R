    ...
    for (currentParameter in namesParameters) {
      ...
      if (conditionsSatisfied(parameters, configurationTable[k, ], 
        ...
        return(NULL)
          ...
          # We ignore the value. Perhaps we could do something with it.
          configurationTable[k, currentParameter] <- NA
        }
      }
    }
  }
...
readScenario <- function(filename = "", scenario = list())
{
  ...
  pathParams <- setdiff(.irace.params.def[.irace.params.def[, "type"] == "p",
                                          "name"], "logFile")
  filename <- path.rel2abs(filename)
...
checkScenario <- function(scenario = defaultScenario())
{
  ...
  if (!is.null.or.empty(scenario$runlog)) {
    scenario$runlog <- path.rel2abs(scenario$runlog, cwd = scenario$execDir)
  }
  ...
  scenario$targetErrorMode <- tolower(scenario$targetErrorMode)
  if (scenario$targetErrorMode == "retry" && scenario$targetRunnerRetries <= 0) {
    irace.error ("when targetErrorMode == 'retry', targetRunnerRetries must be > 0.")
  }
  if (! (scenario$targetErrorMode %in% c("quit", "stop", "retry"))) {
    irace.error ("invalid value '", scenario$targetErrorMode,
                 "' of 'targetErrorMode' (", .irace.params.def["targetErrorMode", "long"],
                 "), valid values are: quit, stop, retry.")
  }
