...
#	puts ARGV.join(" --- ")
	...
	out.puts "\n======================================================================================================"
	out.puts "PARAM_ILS version 2.3.7, copyright Frank Hutter, 2004-2013."
...
if @scenariofile
	scenarioparams = {"algo" => @algo, "run_obj"=>@run_obj, "overall_obj"=>@overall_obj, "cutoff_time"=>@cutoff_time, 
			"runcount_limit" => @maxEvals,
			"cutoff_length"=>@cutoff_length, "tuner-timeout"=>@tunerTimeout, "deterministic"=>deterministic,
			...
			"execdir"=>$execdir, "outdir"=>@outdir, "paramfile"=>$param_file, "feature_file"=>nil, "wallclock_limit"=>@maxWallTime} # FH: added feature_file for convenient reading of SMAC scenario files
	...
	@maxEvals = scenarioparams["runcount_limit"].to_i
	@cutoff_length = scenarioparams["cutoff_length"]
	@tunerTimeout = scenarioparams["tuner-timeout"].to_f
	...
	@maxWallTime = scenarioparams["wallclock_limit"]

	if ((@run_obj =~ /quality/ || @run_obj =~ /approx/) && 
...
puts "tunerTimeout = " +  @tunerTimeout.to_s
puts "max wall time = " + @maxWallTime.to_s
puts "maxEvals = " +  @maxEvals.to_s
